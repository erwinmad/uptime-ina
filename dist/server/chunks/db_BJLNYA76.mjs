...
function initDb() {
  ...
  `);
	...
	try {
		if (db.prepare("SELECT COUNT(*) as count FROM probe_nodes").get().count === 0) {
	...
	if (db.prepare("SELECT COUNT(*) as count FROM system_settings").get().count === 0) for (const [k, v] of [
		["app_name", "Uptime CJR"],
...
function getBrandingSettings() {
	...
	try {
		...
		for (const r of rows) if (r.key in defaults) defaults[r.key] = r.value;
	} catch (err) {
		console.error("Error fetching branding settings:", err);
	}
	return defaults;
}
function updateBrandingSettings(settings) {
	const stmt = db.prepare(`
    INSERT INTO system_settings (key, value, updated_at) 
    VALUES (?, ?, datetime('now'))
    ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = datetime('now')
  `);
	for (const [k, v] of Object.entries(settings)) if (v !== void 0 && v !== null) stmt.run(k, String(v));
}
...
initDb();
//#endregion
export { hashPassword as a, validateSession as c, getBrandingSettings as i, verifyPassword as l, db as n, updateBrandingSettings as o, deleteSession as r, updateUserPassword as s, createSession as t };
