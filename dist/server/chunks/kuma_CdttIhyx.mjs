...
//#region src/pages/api/v1/import/kuma.ts
var kuma_exports = /* @__PURE__ */ __exportAll({ POST: () => POST });
var POST = async ({ request, cookies }) => {
	try {
		const token = cookies.get("sentinel_session")?.value || request.headers.get("authorization")?.replace(/^Bearer\s+/i, "");
		const session = token ? validateSession(token) : null;
		if (!session) return new Response(JSON.stringify({ error: "Akses ditolak. Silakan login terlebih dahulu." }), { status: 401 });
		const payload = await request.json();
		const monitorsList = Array.isArray(payload) ? payload : payload.monitors || payload.monitorList || [];
		if (!Array.isArray(monitorsList) || monitorsList.length === 0) return new Response(JSON.stringify({ error: "Data impor kosong atau format tidak valid." }), { status: 400 });
		let importedCount = 0;
    ...
    `);
		for (const item of monitorsList) {
			...
			else if (rawType.includes("ping")) type = "tcp";
		...
		logAudit("monitors.imported_kuma", "monitor", "bulk", {
			count: importedCount,
...
//#endregion
//#region \0virtual:astro:page:src/pages/api/v1/import/kuma@_@ts
var page = () => kuma_exports;
//#endregion
export { page };
